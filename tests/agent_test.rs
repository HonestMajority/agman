...
use agman::assistant::{validate_agent_attachment, AgentAttachment, AgentKind, Assistant};
...
#[test]
fn agent_model_engineer_requires_task_attachment() {
    let unattached = validate_agent_attachment(&AgentKind::Engineer, &AgentAttachment::Unattached);
    assert!(unattached.is_err());

    let attached = AgentAttachment::Task {
        task_id: "repo--branch".to_string(),
        role_label: Some("primary".to_string()),
    };
    validate_agent_attachment(&AgentKind::Engineer, &attached).unwrap();
}
...
fn agent_model_serializes_engineer_attachment() {
    ...
    let agent = Assistant::create_with_attachment(
        &config,
        "project",
    ...
    let loaded = Assistant::load(agent.dir).unwrap();
